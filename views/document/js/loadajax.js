    ...
    var getTypeindex = function(){
        ...
        $.post('/search/getfields/','typedocment=' + $("#typedocment").val(),function(datoi){
            ...
            html='<script type="text/javascript">$(function () {$( "#datepicker6" ).datepicker({dateFormat: "yy-mm-dd"});});</script>'
                    +'<script>$("#number").keydown(function (e) {this.value = (this.value + "").replace(/[^0-9]/g, "");});</script>';
            ...
            for(var k = 0; k < datoi.length; k++){
               ...
               if(datoi[k].id_type==4){
                    ...
                    if(datoi[k].id_type==2){
                        html=html+'<div class=""><input type="text" id="number" class="form-control" placeholder="'+datoi[k].indexes+'" name="index_'+datoi[k].id_index+'"></div>';
                    }else{
                        if(datoi[k].id_type==3){
                            html=html+'<div class=""><input type="text" id="email" class="form-control" placeholder="'+datoi[k].indexes+'" name="index_'+datoi[k].id_index+'"></div>';
                        }else{
